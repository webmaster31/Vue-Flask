...
import pymysql
from lib.utils.tokens import generate_recovery_codes
from lib.utils import get_random_string, get_pusher_client
...
class ConnectionContext():
    ...
    def __enter__(self):
        if not self.connection:
            ...
            self.connection = pymysql.connect(**pymysql_connect_kwargs)
...
class VersionedModel:
    ...
    def _notify_object_save(new_entity, table_name, operation):
        from tasks.handle_object_save import handle_object_save
        handle_object_save.apply_async(args=[table_name, operation], kwargs=new_entity.get_as_dict())
