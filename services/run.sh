...
cp "$SERVICE_ROOT/$ENV.env" "$API_ROOT/temp.env" &> /dev/null  || { echo  -e "${RED} 👉 No $ENV.env found inside $SERVICE_ROOT ${CLEAR}\n"; exit 1; }
cp "$SERVICE_ROOT/$ENV.env" "$SERVICE_ROOT/temp.env" &> /dev/null  || { echo  -e "${RED} 👉 No $ENV.env found inside $SERVICE_ROOT ${CLEAR}\n"; exit 1; }
for service_dir in $SERVICE_ROOT/*_service
do
  cp "$SERVICE_ROOT/$ENV.env" "$service_dir/temp.env" &> /dev/null  || { echo  -e "${RED} 👉 No $ENV.env found inside $SERVICE_ROOT ${CLEAR}\n"; exit 1; }
done
...
# validate env files
bash "$SERVICE_ROOT/scripts/validate_env.sh" "$SERVICE_ROOT/example.env" "$SERVICE_ROOT/temp.env"; exit_status_service=$?
for service_dir in $SERVICE_ROOT/*_service
do
  bash "$SERVICE_ROOT/scripts/validate_env.sh" "$service_dir/.env.dev" "$service_dir/temp.env"; exit_status_service=$?
done
...
bash "$SERVICE_ROOT"/scripts/build_env.sh "$SERVICE_ROOT/example.env" "$SERVICE_ROOT/temp.env" "$ENV" >> "$SERVICE_ROOT"/.env

for service_dir in $SERVICE_ROOT/*_service
do
  rm -rf "$service_dir"/.env || true
  bash "$SERVICE_ROOT"/scripts/build_env.sh "$service_dir/.env.dev" "$service_dir/temp.env" "$ENV" >> "$service_dir"/.env
done
...
for service_dir in $SERVICE_ROOT/*_service
do
  rm -rf "$service_dir/temp.env" || true
done
